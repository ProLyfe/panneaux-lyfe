...
const ModalQuizz = ({ navigation, route }) => {
  ...
  const verifyAnswer = (answer, index) => {
    ...
    if (answer === true) {
      ...
      console.log('randomizedArray :', randomizedArray[questionNb].id)
    } else {
      console.log('Nop')
      // setBgColor('red')
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.answersContainer}>
        {/* <TouchableOpacity style={{...styles.answers, backgroundColor: bgColor}} onPress={() => verifyAnswer(true)}><View><Text style={styles.textAnswers}>Vitesse limitée à 50Km/h</Text></View></TouchableOpacity>
        <TouchableOpacity style={{...styles.answers, backgroundColor: bgColor}} onPress={() =>verifyAnswer(false)}><View><Text style={styles.textAnswers}>Vitesse limitée à 90Km/h</Text></View></TouchableOpacity>
        <TouchableOpacity style={{...styles.answers, backgroundColor: bgColor}} onPress={() => verifyAnswer(false)}><View><Text style={styles.textAnswers}>Vitesse limitée à 100Km/h</Text></View></TouchableOpacity>
        <TouchableOpacity style={{...styles.answers, backgroundColor: bgColor}} onPress={() => verifyAnswer(false)}><View><Text style={styles.textAnswers}>Vitesse limitée à 120Km/h</Text></View></TouchableOpacity> */}
        
        {questions.map((question, index) => <TouchableOpacity style={styles.answers} onPress={() => verifyAnswer(question.status, index)} key={question.questionId}><View><Text style={styles.textAnswers}>{question.proposition}</Text></View></TouchableOpacity>)}
